...
const Pagination: React.FC<PaginationProps> = ({ currentPage, totalPages, onPageChange }) => {
  ...
  return (
    <div className="p-4 sm:px-6 xl:px-7.5">
      <nav>
        <ul className="flex flex-wrap items-center justify-center">
          <li>
            <button
              className="flex h-8 w-8 items-center justify-center rounded hover:bg-gray2 hover:text-white"
          ...
          {pageNumbers.map((page) => (
            <li key={page}>
              <button
                className={`flex items-center justify-center rounded py-1.5 px-3 font-medium hover:bg-gray2 hover:text-white ${
                  page === currentPage ? 'bg-gray2 text-white' : ''
                }`}
